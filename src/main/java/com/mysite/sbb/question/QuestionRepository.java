...
public interface QuestionRepository extends JpaRepository<Question, Integer> {
	...
	// JPA의 CRUD 메소드 : 
			...
			// findAll() 
			// findById()
			// 정의해서 사용 : findBySubject() ....기타 등등
	
	//Question 테이블을 SQL 쿼리를 사용하지않고 JPA 메소드를 사용해서 CRUD 하는 Repository 
	...
	//페이징을 처리하기 위한 메서드 생성
	// select * from question : Pagealbe 변수에 : page, 레코드 수를 넣어주면(ex:10개)
	// JPA에서 페이지 번호는 0부터 시작
	// 출력할 레코드 수를 JPA에 알려주면 내부에서 JPA가 전체 레코드(1000개) / 10 = 100페이지가 나온다. 
