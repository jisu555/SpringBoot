...
class SbbApplicationTests {
	...
	@Autowired	 //객체 자동 주입, JAP의 CRUD할 수 있는 메소드가 적용되어 있음.
	...
	*/
	...
	public void testjpa8() {
		...
		//Question 객체의 answerList컬럼은 List<answer>
		List<Answer> all = 
				q.getAnswerList();
	...
	*/
	...
	public void testjpa7() {
		...
		if (op.isPresent()) {	//isPresent() : null일 경우 (false), null이 아니면 (true)
			Answer a = op.get();
			System.out.println(a.getId());
			System.out.println(a.getContent());
			System.out.println(a.getCreateDate());
	...
	*/
	...
	public void testAnswerjpa() {
		// 1. Question(부모) 테이블의 답변을 처리할 레코드를 먼저 select한다. findById(1)
		Optional <Question> op =
				this.questionRepository.findById(3);
		Question q = op.get();
	...
	*/
	...
	/* 사용자 정의 select 문 : subject 컬럼, content 컬럼 검색, Like 사용
	@Test
	public void testjpa () {
